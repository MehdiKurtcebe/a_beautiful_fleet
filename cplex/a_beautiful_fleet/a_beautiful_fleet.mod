...
execute {
  ...
  // her guzellestiricinin hareketlerinin uzerinden gec ve yazdir
  for (var b in B) {
    for (var a in A) {
      if (x[b][a] > 0) {
        f.write(b + "," + a.nFrom.z + "," + a.nFrom.t + "," + a.nTo.z + "," + a.nTo.t + "," + a.pi + ",");
        if (A_BEAU.contains(a)) {
          f.writeln("Beautification");
        } else if (A_HOT.contains(a)) {
          f.writeln("Bring-to-Hotspot");
        } else if (A_MOVE.contains(a)) {
          f.writeln("Move to zone " + a.nTo.z);
