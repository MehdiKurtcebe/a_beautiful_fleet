...
profits = {
    ...
    "HOT": 10.0,
    "WAIT": 0.0,
    "MOVE": np.array(piMOVE),
}  # be sure movement is negative
...
class Ant:
    ...
    def choose_action(self, current_zone, current_time):
        ...
        for z2 in range(1, zones + 1):
            if (
                current_zone != z2 and n[z2 - 1] + aco_nOut[z2 - 1] > 1
            ):  # bigger than 1 because to be sure not move for nothing
                move_cost = mMOVE[current_zone - 1][z2 - 1]
                if (
                    current_time + move_cost < t_max
                    and float(n[z2 - 1] + aco_nOut[z2 - 1]) / float(move_cost / mBEAU)
                    > 1.0
                ):  # ratio is better checking option
        ...
        for action in actions:
            ...
            probabilities.append(
                (pheromone**pheromone_importance) * (heuristic**heuristic_importance)
            )
        ...
        for aco_time in range(t_max):
            for b in range(beautificators):
                if self.action_durations[b] == 0:  # Check if action is not in progress
                    ...
                    self.total_profits[b] += heuristic_profit(
                        action_type, self.current_zones[b], next_zone
                    )
                    # nout - nhot - n
                    #  5      3     8
                    # beau  5      -     7
                    ...
                    if action_type == "BEAU":
                        ...
                        if (
                            n[self.current_zones[b] - 1]
                            < aco_nOut[self.current_zones[b] - 1]
                        ):
                            aco_nOut[self.current_zones[b] - 1] = n[
                                self.current_zones[b] - 1
                            ]
...
def ant_colony_optimization():
    ...
    for iteration in range(num_iterations):
        ...
        for ant in ants:
            total_profit = sum(ant.total_profits)
        ...
        # Calculate and sort total profits of all ants
        ant_profits = [sum(ant.total_profits) for ant in ants]

        sorted_ants = sorted(
            zip(ants, ant_profits), key=lambda item: item[1], reverse=True
        )

        # Add the most pheromone to the best ant, decreasing amounts for subsequent ants
        for i, (ant, profit) in enumerate(sorted_ants):
            pheromone_amount = pheromone_increment * (
                profit / best_total_profit
            )  # Decreasing pheromone amount
            for b in range(beautificators):
                for _, z1, aco_time, z2, _, action in ant.paths[b]:
                    if action == "MOVE":
                        pheromones[(z1, z2, aco_time)]["MOVE"] += (
                            move_pheromone_decrement * pheromone_amount
                        )
                    else:
...
# Output Results
script_dir = os.path.dirname(os.path.realpath(__file__))
output_dir = os.path.join(script_dir, "..", "csv-outputs")

for b in range(beautificators):
    output_file = os.path.join(output_dir, f"output_{b + 1}.csv")
    with open(output_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["Beautificator", "Zone_From", "Time_From", "Zone_To", "Time_To", "Action"]
        )
...
script_dir = os.path.dirname(os.path.realpath(__file__))
geojson_file = os.path.join(script_dir, "map-izmit.geojson")
...
script_dir = os.path.dirname(os.path.realpath(__file__))
csv_outputs_dir = os.path.join(script_dir, "..", "csv-outputs")
        ...
        if action in ("BEAU", "HOT"):
            ...
            if zone_polygon:
                ...
                if randomPointBefore is not None:
                    AntPath(
                        [randomPointBefore, point], color="blue", weight=2.5, opacity=1
                    ).add_to(m)
